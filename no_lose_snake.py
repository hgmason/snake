...
def draw_box(x,y, a = scalar, color = GREEN):
    #(left, top, width, height)
    a = int(size*a)
    pygame.draw.rect(windowSurface, color, (y,x,a,a))
def clear_box(x,y):
    a = int(size*scalar)
...
def draw_existing_food():
    global food
    a = int(size*scalar)
    pygame.draw.rect(windowSurface, RED, (food[1],food[0],a,a))
...
def move_snake():
    ...
    if (next_box[0] >= height) or (next_box[1] >= width) or (next_box[0] < 0) or (next_box[1] < 0):
        end_game()
        return
    ...
    windowSurface.fill(BLACK)
    head_color = (1/3, 1.0, 1)
    for i in range(len(snake)):
        hue = (1/3 + i/len(snake))%1
        rgb = colorsys.hsv_to_rgb(hue, 1, 1)
        rgb = [rgb[0]*255,rgb[1]*255,rgb[2]*255]
        item = snake[i]
        draw_box(item[0],item[1],a = (scalar - i/len(snake)), color = rgb)
    draw_existing_food()
...
snake = []
for i in range(height*width//size//size):
...
while True:
    for event in pygame.event.get():
        ...
        '''
        if event.type == KeyUp:
            if (event.key == UP_KEY) or (event.key == DOWN_KEY) or (event.key == RIGHT_KEY) or (event.key == LEFT_KEY):
                pressed = pressed - 1
            if pressed <= 0:
                game_running = False
                pressed = 0
